...
import com.monkeylluffy.recorder.cheapfile.CheapSoundFile.Factory;
 ...
 * 
 * 
 * cheapmp3代表一个MP3文件做一个“cheap”的扫描文件，解析帧头只得到每一帧的音量非常粗略的估计。
 * 有用的单元测试可能是在不同的地方寻找同步：
 * FF FA
 * FF FB
 * 00 FF FA
 * FF FF FA
...
public class CheapMP3 extends CheapSoundFile {
     ...
     * MP3支持查找到的文件的中间，没有头，所以这种方法支持听到什么“cut”的文件听起来不实际文件保存到磁盘的第一个。
     */
