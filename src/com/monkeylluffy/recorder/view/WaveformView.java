...
/**
 * waveformview是一个Android视图显示的可视化表示音频波形。它从一个cheapsoundfile检索框架的收益对象并重新计算轮廓线在几个缩放级别。
 * 这个类不处理选择或任何直接的触摸交互，因此它公开了一个侦听器接口。类，将该视图应该添加本身作为一个听众，使视图滚动
 * waveformview实际上并没有处理的选择，但它只会显示不同颜色的波形的选定部分。
 */
